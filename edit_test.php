<?php
// edit_test.php - с поддержкой страниц и валидацией
...
// Преобразуем страницы в JSON для JavaScript
$pagesJson = json_encode($test['pages'] ?? [], JSON_UNESCAPED_UNICODE);

// Если страниц нет, добавляем одну
if (empty($test['pages'])) {
    $test['pages'] = [
        [
            'id' => 'page_1',
    ...
    <form method="POST" id="testForm" enctype="multipart/form-data">
        <div class="card">
            ...
            <div class="form-group">
                ...
                <textarea name="description" class="form-control"><?= htmlspecialchars($test['description'] ?? '') ?></textarea>
            </div>
            
            <div class="form-group">
                <label>Статус</label>
                <select name="status" class="status-select">
                    <option value="draft" <?= ($test['status'] ?? '') === 'draft' ? 'selected' : '' ?>>Черновик</option>
                    <option value="active" <?= ($test['status'] ?? '') === 'active' ? 'selected' : '' ?>>Активный</option>
                    <option value="archived" <?= ($test['status'] ?? '') === 'archived' ? 'selected' : '' ?>>Архивирован</option>
                </select>
            </div>
        </div>
        
        <div class="card">
            <h3 class="card-title">📄 Страницы и вопросы <span class="question-count" id="pageCount"></span></h3>
...
// Вычисляем максимальные счетчики
pages.forEach(page => {
    const num = parseInt(page.id.replace('page_', ''));
    if (num > pageCounter) pageCounter = num;
    if (page.questions) {
        page.questions.forEach(q => {
            const qNum = parseInt(q.id.replace('q_', ''));
            if (qNum > questionCounter) questionCounter = qNum;
        });
    }
});
...
function renderPages() {
    ...
    document.getElementById('pageCount').textContent = '(' + pages.length + ' страниц)';
    updateJson();
}
...
function updateJson() {
    document.getElementById('pagesJson').value = JSON.stringify(pages);
}
